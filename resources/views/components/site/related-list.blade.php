            ...
            @foreach($items as $item)
                <li>
                    <a href="{{ $item['url'] }}" class="surface-card group block p-5">
                        <p class="font-sans font-semibold text-xl text-neutral-100 group-hover:text-accent tracking-tight transition-colors">{{ $item['title'] }}</p>
                        <p class="text-neutral-400 text-sm mt-2 leading-relaxed line-clamp-2">{{ $item['excerpt'] }}</p>
                    </a>
                </li>
            @endforeach
        </ul>
