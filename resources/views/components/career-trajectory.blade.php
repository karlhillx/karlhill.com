...
<section id="career-trajectory" class="relative overflow-hidden bg-gradient-to-b from-white to-gray-50 px-0 py-16 sm:py-20 md:py-24">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12 sm:mb-16">
            <h2 class="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900 md:text-5xl mb-4">
                <span class="inline-block text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-cyan-600">
                    Career Trajectory
                </span>
            </h2>
            ...
            <div class="space-y-12">
                @foreach($career as $index => $position)
                    <div class="relative flex items-start md:items-center">
                        ...
                        <div class="absolute left-6 md:left-1/2 w-4 h-4 rounded-full bg-gradient-to-r from-indigo-500 to-cyan-500 border-4 border-white z-10 shadow-lg career-timeline-dot"
                             style="animation-delay: {{ $index * 0.3 }}s"></div>

                        <!-- Content card -->
                        <div class="ml-16 md:ml-0 md:w-1/2 {{ $index % 2 === 0 ? 'md:mr-auto md:pr-8' : 'md:ml-auto md:pl-8' }}">
                            <div class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-300 border border-gray-200 career-card">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between mb-1">
                                            ...
                                            <span class="text-xs font-semibold px-2 py-1 rounded-full career-badge
                                                @if($position['level'] === 'staff') bg-purple-100 text-purple-800
                                                @elseif($position['level'] === 'lead') bg-indigo-100 text-indigo-800
                                                @elseif($position['level'] === 'senior') bg-cyan-100 text-cyan-800
                                        ...
                                        <p class="text-sm text-gray-500 mt-1">{{ $position['year'] }}</p>
                                        @if(isset($position['location']))
                                            <p class="text-xs text-gray-400 mt-1">{{ $position['location'] }}</p>
                                        @endif
                                    </div>
                                </div>
                                @if(isset($position['description']))
                                    <p class="text-sm text-gray-600 mb-4 leading-relaxed">{{ $position['description'] }}</p>
                                @endif
                                <ul class="space-y-2 mt-4">
                                    @foreach($position['highlights'] as $highlightIndex => $highlight)
                                        <li class="flex items-start gap-2 text-sm text-gray-600">
                                            <svg class="w-5 h-5 text-indigo-500 flex-shrink-0 mt-0.5 career-check-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
