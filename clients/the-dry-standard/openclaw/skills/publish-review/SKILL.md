...
4. Write `content/reviews/{slug}.md` using the `write-review` skill and `data/schema/review.schema.yaml`. `product` is the SKU only; `title` is usually brand + product. Do not repeat the brand in `product` unless it is the official SKU name. The markdown body is the essay — not a product overview, not a retailer blurb.
