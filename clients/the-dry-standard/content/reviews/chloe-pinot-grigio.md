---
title: 'Chloe Alcohol-Removed Pinot Grigio'
slug: chloe-pinot-grigio
id: TDS-0082
brand: 'Chloe Wine Collection'
product: 'Alcohol-Removed Pinot Grigio'
abv: 0.5%
abv_numeric: 0.5
category: wine
subcategory: 'pinot grigio'
country: 'United States'
region: California
style: 'Alcohol-removed Pinot Grigio'
production_type: dealcoholized
method_facet: unpublished
verified: 'yes'
production_note: 'Chloe brands this as alcohol-removed California Pinot Grigio. Named removal technology is unpublished. Exact ABV is not on the current Chloe product page; commercial product data lists 0.5% ABV.'
base_beverage: 'Award-winning Chloe Pinot Grigio, now alcohol-removed'
producer: 'Chloe Wine Collection (The Wine Group; Acampo & Ripon, CA)'
volume: '750 ml'
calories: '25 kcal per 5 fl oz (producer average analysis)'
sugar: '2 g sugars per 5 fl oz (producer average analysis)'
availability: 'US retail including Total Wine, Meijer, and Publix'
review_date: '2026-09-18'
updated_date: '2026-09-20'
rating: 75
verdict: "The zero-proof version of Chloe's approachable Pinot Grigio — light, citrusy, and priced for the everyday shelf."
summary: "Chloe's Alcohol-Removed Pinot Grigio is a dealcoholized California white: light citrus and green apple. Commercial listings put ABV at 0.5%; Chloe's page confirms alcohol-removed without a number."
tastes:
  - citrus
  - lemon
  - lime
  - 'green apple'
profile:
  - 'Bright acidity'
  - 'Light body'
  - 'Clean finish'
nose: 'Citrus and green apple — the Pinot Grigio aromatic signature, clean and light.'
palate: 'Crisp and easy-drinking; the fruit is simple and pleasant, the body lighter than a full-strength version.'
finish: 'Clean and brief.'
best_for: "Everyday white-wine occasions, the bottle that doesn't need to be special."
serve: 'Well chilled.'
image: media/reviews/chloe-pinot-grigio.jpg
image_alt: 'Chloe Alcohol-Removed Pinot Grigio bottle'
image_credit: 'Product photo via chloewinecollection.com'
image_source: producer
image_source_url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
image_sku_confirmed: 'yes'
discrepancies:
  -
    field: abv
    note: "Chloe's public product page confirms alcohol-removed Pinot Grigio but does not publish a numerical ABV. Meijer product features list 0.5% ABV. Publix product copy says contains less than 0.5% alcohol by volume. We print 0.5% from current commercial product data and keep the ceiling claim visible here."
sources:
  -
    title: 'Alcohol-Removed Pinot Grigio — Chloe Wine Collection'
    url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
    claims:
      - dealcoholized
      - production_type
      - base_beverage
      - producer
      - country
      - region
      - calories
      - sugar
  -
    title: 'Chloe Alcohol-Removed Pinot Grigio 750ml — Meijer'
    url: 'https://www.meijer.com/shopping/product/chloe-alcohol-removed-pinot-grigio-750ml/8130801633.html'
    claims:
      - abv
      - volume
      - availability
  -
    title: 'Chloe Alcohol-Removed Pinot Grigio — Publix'
    url: 'https://www.publix.com/pd/chloe-alcohol-removed-pinot-grigio/RIO-PCI-637805'
    claims:
      - dealcoholized
      - producer
      - country
      - region
      - availability
status: published
product_id: TDS-0082
identifiers:
  -
    type: tds
    value: TDS-0082
sensory:
  -
    descriptor: citrus
    locations:
      - palate
  -
    descriptor: lemon
    locations:
      - palate
  -
    descriptor: lime
    locations:
      - palate
  -
    descriptor: green_apple
    locations:
      - palate
structure_scales:
  acidity: 3
  body: 1
  finish_length: 0
provenance:
  production_type:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
    note: 'Producer page brands the wine Alcohol-Removed; named removal technology undeclared.'
  base_beverage:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
  producer:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
  country:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
  region:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
  calories:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
  sugar:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://www.chloewinecollection.com/wines/alcohol-removed-pinot-grigio/'
  abv:
    kind: distributor
    confidence: secondary
    url: 'https://www.meijer.com/shopping/product/chloe-alcohol-removed-pinot-grigio-750ml/8130801633.html'
    note: 'Exact 0.5% ABV from commercial product data (Meijer listing). Not currently published as a number on Chloe’s product page.'
  volume:
    kind: retailer
    confidence: secondary
    url: 'https://www.meijer.com/shopping/product/chloe-alcohol-removed-pinot-grigio-750ml/8130801633.html'
  availability:
    kind: retailer
    confidence: secondary
    url: 'https://www.meijer.com/shopping/product/chloe-alcohol-removed-pinot-grigio-750ml/8130801633.html'
---

Chloe Alcohol-Removed Pinot Grigio is dealcoholized California Pinot Grigio. Chloe’s page confirms alcohol removal without naming the machine; commercial product data puts the bottle at 0.5% ABV.

The nose is citrus and green apple — the Pinot Grigio aromatic signature, clean and light.

On the palate, crisp and easy-drinking; the fruit is simple and pleasant, the body lighter than a full-strength version.

The finish is clean and brief. The everyday Chloe white without the ethanol — light, citrusy, and priced for the shelf rather than the cellar.
