---
title: 'Dr. Heidemanns-Bergweiler Riesling'
slug: heidemanns-bergweiler-riesling
id: TDS-0017
ean: '0883122400114'
brand: 'Dr. Heidemanns Bergweiler'
product: 'Non-Alcoholic Riesling'
abv: 0.0%
abv_numeric: 0.0
abv_qualifier: exact
category: wine
subcategory: riesling
country: Germany
region: Mosel
style: 'Dealcoholized Mosel Riesling'
production_type: dealcoholized
verified: 'yes'
production_note: 'Retail catalog copy describes a dealcoholized German Riesling. Named removal technology is unpublished — the estate factsheets do not cover this alcohol-free SKU. Do not transfer the Noble House thermal-evaporation sheet to this bottle. ABV 0.0% is retailer-sourced (Gedonist), not producer-verified.'
image: media/reviews/heidemanns-bergweiler-riesling.jpg
image_alt: 'Dr. Heidemanns-Bergweiler Non-Alcoholic Riesling'
image_credit: 'Product photo via Dr. Heidemanns-Bergweiler'
image_source: editorial
image_sku_confirmed: 'yes'
review_date: '2026-09-18'
updated_date: '2026-09-20'
rating: 79
verdict: 'A Mosel name on an NA Riesling. Citrus and a little residual sweetness, thinner than Leitz, better documented origin than most Total Wine whites.'
summary: 'Dr. Heidemanns-Bergweiler dealcoholized Mosel Riesling. Gedonist lists strength 0.0%; the winery site does not publish this SKU’s residual ABV or removal method.'
discrepancies:
  -
    field: abv
    note: 'Gedonist lists Dr. Heidemanns-Bergweiler Riesling Alcohol Free 0.75L at 0.0 strength. The Total Wine / RoomBox catalog for the same non-alcoholic Riesling leaves ABV as N/A. The estate site has no alcohol-free Riesling factsheet. We print 0.0% from Gedonist as secondary retailer data and do not treat it as producer-verified.'
tastes:
  - lime
  - 'green apple'
  - 'slate note'
profile:
  - Off-dry
  - 'Bright acidity'
  - 'Light body'
  - 'Short finish'
nose: 'Lime and green apple, a faint slate note.'
palate: 'Off-dry, light, Mosel-shaped acidity without ethanol weight.'
finish: 'Short citrus. The sugar shows as it warms.'
best_for: 'Weeknight fish when you want a German Riesling label.'
serve: 'Well chilled.'
sources:
  -
    title: 'Dr. Heidemanns-Bergweiler — estate'
    url: 'https://heidemanns-bergweiler.com/'
    claims:
      - producer
      - origin
      - region
      - country
  -
    title: 'Вино Dr. Heidemanns-Bergweiler Riesling Alcohol Free 0.75л — Gedonist'
    url: 'https://gedonist.com.ua/ru/vyno-dr.-heidemanns-bergweiler-riesling-alcohol-free-0.75l-nimechchyna/'
    claims:
      - abv
      - volume
  -
    title: 'Dr Heidemanns German Riesling Non-Alcoholic Wine — RoomBox / Total Wine catalog'
    url: 'https://www.orderroombox.com/product/dr-heidemanns-german-riesling-non-alcoholic-wine/'
    claims:
      - dealcoholized
      - production_type
status: published
product_id: TDS-0017
identifiers:
  -
    type: ean
    value: '0883122400114'
    source: 'packaging or producer listing'
  -
    type: tds
    value: TDS-0017
sensory:
  -
    descriptor: lime
    locations:
      - palate
  -
    descriptor: green_apple
    locations:
      - palate
  -
    descriptor: slate
    locations:
      - palate
structure_scales:
  sweetness: 2
  acidity: 3
  body: 1
  finish_length: 0
provenance:
  producer:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://heidemanns-bergweiler.com/'
  country:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://heidemanns-bergweiler.com/'
  region:
    kind: manufacturer
    confidence: manufacturer_verified
    url: 'https://heidemanns-bergweiler.com/'
  abv:
    kind: retailer
    confidence: secondary
    url: 'https://gedonist.com.ua/ru/vyno-dr.-heidemanns-bergweiler-riesling-alcohol-free-0.75l-nimechchyna/'
    note: 'Gedonist: Alcohol Free 0.75L listed at 0.0 strength. Not on the estate factsheets.'
  production_type:
    kind: retailer
    confidence: secondary
    url: 'https://www.orderroombox.com/product/dr-heidemanns-german-riesling-non-alcoholic-wine/'
    note: 'Total Wine / RoomBox catalog: dealcoholized German Riesling. Method unpublished.'
  ean:
    kind: label
    confidence: label_verified
    note: 'Barcode recorded from packaging or producer listing'
---

Dr. Heidemanns-Bergweiler Alcohol-Free Riesling is dealcoholized from Mosel, Germany. Total Wine / RoomBox call it dealcoholized; Gedonist lists strength 0.0%. The estate site publishes vineyard factsheets but not this alcohol-free SKU’s residual ABV or removal machine, so the method stays unpublished. The glass is a light, off-dry Riesling. Not Eins-Zwei-Zero. A Mosel-shaped supermarket white.

The nose is lime and green apple, a faint slate note.

On the palate, off-dry, light, Mosel-shaped acidity without ethanol weight.

The finish is short citrus. The sugar shows as it warms. A Mosel name on an NA Riesling — citrus and a little residual sweetness, thinner than Leitz, better documented origin than most Total Wine whites.
