---
title: "Nonny Czech Pilsner"
slug: nonny-czech-pilsner
id: TDS-0047
brand: Nonny Beer
product: Czech Pilsner
category: beer
subcategory: Czech-style pilsner
country: Czech Republic
style: Non-alcoholic Czech-style craft pilsner
abv: "<0.5% ABV (producer)"
abv_numeric: 0.5
dealcoholized: not-verified
dealcoholized_note: "Nonny publishes the beer's ingredients and nutrition but not its production method; whether alcohol is removed after fermentation or limited during it is not documented."
calories: "42 calories per can; 9 g carbs, 0 g sugar (producer)"
ingredients: "Water, barley, hops, yeast (producer)"
producer: Nonny Beer
availability: "US via us.nonny.beer and specialty NA retailers including Upside Drinks"
review_date: "2026-09-18"
updated_date: "2026-09-18"
rating: 83
verdict: "A Czech pilsner made by people who clearly care about Czech pilsner: the hop bitterness is right, the malt is clean, and at 42 calories a can it's the leanest credible pils on this desk."
summary: "Nonny's Czech Pilsner is a traditional-ingredient NA lager from the Czech Republic: water, barley, hops, yeast, 42 calories a can, real bitterness."
nose: "Fresh Saaz-style hop grassiness and clean pilsner malt — the classic Czech aromatic signature."
palate: "Crisp and properly bitter; grassy hops over a light, clean malt body with genuine pilsner bite."
finish: "Dry, bitter and refreshing — the hop character carries a long way for a 42-calorie can."
best_for: "Anyone who drinks Czech lager and wants the zero-proof version to taste like Czech lager."
serve: "Cold, in a pils glass."
image: media/reviews/nonny-czech-pilsner.jpg
image_alt: "Nonny Czech Pilsner can"
image_credit: "Product photo via us.nonny.beer"
sources:
  - title: "Nonny Czech Pilsner — Nonny Beer (US)"
    url: https://us.nonny.beer/products/nonny-czech-pilsner
    claims: [abv, calories, ingredients, producer, country, availability, dealcoholized]
status: published
---

Nonny makes its Czech Pilsner from four ingredients — water, barley, hops, yeast — and gets it to 42 calories a can, which is remarkable discipline for a beer that actually tastes like beer. The Czech Republic is the one country where the NA category has decades of institutional knowledge, and it shows here: the hop bitterness is right (grassy, present, not apologetic), the malt is clean, and the finish is dry rather than sweet.

What the producer won't tell you is how the beer gets under 0.5%, so the dealcoholized field stays not-verified — a caution that applies across the entire Czech NA sector, where limited-fermentation and dealcoholization approaches both exist and labels rarely distinguish. What the can does say is more useful: four ingredients, 42 calories, and a bitterness that suggests the brewers drank a lot of real Pilsner Urquell while developing it. As a light, honest lager, it earns its place on the shelf.
