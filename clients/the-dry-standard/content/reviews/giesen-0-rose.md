---
title: "Giesen 0% Rosé"
slug: giesen-0-rose
id: TDS-0018
brand: Giesen
product: 0% Rosé
category: wine
subcategory: rosé
country: New Zealand
region: "Marlborough and Waipara"
style: Dealcoholized New Zealand rosé
abv: "<0.5% ABV (producer)"
abv_numeric: 0.5
dealcoholized: yes
dealcoholization_method: "Spinning cone technology — aroma, alcohol, and body separated, then aroma and body recombined"
base_beverage: "Full-strength New Zealand rosé from aromatic and red varietals, then dealcoholized"
producer: Giesen Wines
volume: 750 ml
calories: "13 calories per 100 ml (producer)"
sugar: "18 g/L residual sugar (producer)"
image: media/reviews/giesen-0-rose.jpg
image_alt: "Giesen 0% Rosé"
image_credit: Editorial still
review_date: "2026-09-18"
updated_date: "2026-09-18"
rating: 86
verdict: "The same spinning-cone pipeline as the 0% Sauvignon Blanc and Riesling, applied to a pale New Zealand rosé. White peach and strawberry, dry enough for lunch."
summary: "Giesen ferments a full-strength Marlborough and Waipara rosé, then removes the alcohol with spinning cone technology. Less than 0.5% ABV."
nose: "White peach, apple, and fresh strawberry — lifted, not candied."
palate: "Cranberry and stone fruit over a light frame. 18 g/L residual sugar reads off-dry, not soda."
finish: "Crisp and short. The acidity does the work the ethanol used to."
best_for: "Charcuterie, a warm afternoon, anyone who already likes the Giesen 0% whites."
serve: "Well chilled, white-wine glass."
sources:
  - title: "0% Rosé — Giesen Wines"
    url: https://giesen.co.nz/products/0-rose
    claims: [abv, method, dealcoholized, base_beverage, origin, region, country, volume, calories, sugar, producer]
  - title: "FAQs About Giesen 0% — Giesen Wines"
    url: https://giesen.co.nz/pages/faqs-about-giesen-0
    claims: [method, dealcoholized, abv, base_beverage]
status: published
---

Giesen does not change the story for the rosé. Fruit from Marlborough and Waipara is fermented as wine, then spinning cone technology splits aroma, alcohol, and body. Aroma and body go back together; a little grape juice restores mouthfeel. The producer lists under 0.5% ABV, 13 calories per 100 ml, and 18 g/L residual sugar.

The glass is the same argument as the 0% Riesling: varietal fruit survives the cone. Peach and strawberry, a dry-seeming finish, less weight than a 12% Waipara rosé. If you already trust this pipeline, this is the pink bottle in the set.
